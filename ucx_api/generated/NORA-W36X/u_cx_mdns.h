...
*
* mDNS AT Commands:
*   AT+UWMDNSC=<index>,"<name>","<type>",<port>  - Configure service
*   AT+UWMDNSS                                    - Start responder
*   AT+UWMDNSC?                                   - Query services
*   AT+UWMDNSP                                    - Stop responder
 ...
 * 
 * Output AT command:
 * > AT+UWMDNSC=<index>,"<service_name>","<service_type>",<port>
 *
 * @param[in]  puCxHandle:    uCX API handle
 * @param[in]  index:         Service index (0-based, allows multiple services)
 * @param[in]  service_name:  Service instance name (e.g., "matter-device")
 * @param[in]  service_type:  Service type (e.g., "_matter._tcp" or "_matterc._udp")
 * @param[in]  port:          Port number (1-65535)
 * @return                    0 on success, negative value on error.
 *
 * Example:
 *   uCxMdnsConfigureService(handle, 0, "matter-device", "_matter._tcp", 5540);
 */
int32_t uCxMdnsConfigureService(uCxHandle_t * puCxHandle, int32_t index,
