...
bool uCxWifiStationGetSecurityBegin(uCxHandle_t * puCxHandle, int32_t wlan_handle, uCxWifiStationGetSecurity_t * pWifiStationGetSecurityRsp)
{
    ...
    switch (rspSyntaxVal)
    {
        ...
        case -1:
            ...
            break;
        case 3:
            pWifiStationGetSecurityRsp->type = U_CX_WIFI_STATION_GET_SECURITY_RSP_TYPE_SECURITY_MODE_STR;
            ret = uCxAtUtilParseParamsF(pParamsLine, "-ds", &pWifiStationGetSecurityRsp->rspSecurityModeStr.security_mode, &pWifiStationGetSecurityRsp->rspSecurityModeStr.username, U_CX_AT_UTIL_PARAM_LAST);
            break;
        default:
            return false;
    } /* ~switch (rspSyntaxVal) */
